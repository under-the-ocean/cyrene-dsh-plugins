 ...
 * web GUI. Loads the model from the plugin's own /cyrene-pet/model/* route,
 * the Cubism Core from /cyrene-pet/live2dcubismcore.min.js, and PIXI +
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var POLL_MS = 2000
    var API_STATE = '/api/cyrene-pet/state'
    var API_INFO = '/api/cyrene-pet/info'
    var API_CONFIG = '/api/cyrene-pet/set-config'
    var MODEL_URL = '/cyrene-pet/model/Cyrene.model3.json'
    var CUBISM_CORE_URL = '/cyrene-pet/live2dcubismcore.min.js'
    ...
    function apply(ctx) {
      ...
      fetch('/api/cyrene-pet/settings').then(function (r) { return r.json() }).then(function (s) {
        if (disposed) return
        petSettings = s
      ...
      function loadConfig() {
        fetch('/api/cyrene-pet/config').then(function (res) {
